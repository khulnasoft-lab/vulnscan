...
Vulnscan is a module which enhances nmap to a vulnerability scanner. The
...
You may execute vulnscan with the following argument to use a single
database:
...
Just execute vulnscan like you would by refering to one of the pre-
...
Clone the GitHub repository like this:
   ...
   git clone https://github.com/khulnasoft-lab/vulnscan khulnasoft-lab_vulnscan
   ln -s `pwd`/khulnasoft-lab_vulnscan /usr/share/nmap/scripts/vulnscan    
...
Version detection of vulnscan is only as good as Nmap version detection
...
portrule = function(host, port)
	if port.version.product ~= nil and port.version.product ~= "" then
		...
		stdnse.print_debug(1, "vulnscan: No version detection data available. Analysis not possible.")
	end
end
...
action = function(host, port)
	...
	stdnse.print_debug(1, "vulnscan: Found service " .. prod)

	-- Go into interactive mode
	if nmap.registry.args.vulnscaninteractive == "1" then
		stdnse.print_debug(1, "vulnscan: Enabling interactive mode ...")
		...
		if string.len(prod_override) ~= 0 then
			prod = prod_override
			stdnse.print_debug(1, "vulnscan: Product overwritten as " .. prod)
		end
	end

	-- Read custom report structure
	if nmap.registry.args.vulnscanoutput ~= nil then
		if nmap.registry.args.vulnscanoutput == "details" then
			struct = "[{id}] {title}\nMatches: {matches}, Prod: {product}, Ver: {version}\n{link}\n\n"
		elseif nmap.registry.args.vulnscanoutput == "listid" then
			struct = "{id}\n"
		elseif nmap.registry.args.vulnscanoutput == "listlink" then
			struct = "{link}\n"
		elseif nmap.registry.args.vulnscanoutput == "listtitle" then
			struct = "{title}\n"
		else
			struct = nmap.registry.args.vulnscanoutput
		end

		stdnse.print_debug(1, "vulnscan: Custom output structure defined as " .. struct)
	end

	-- Read custom database link
	if nmap.registry.args.vulnscandblink ~= nil then
		db_link = nmap.registry.args.vulnscandblink
		stdnse.print_debug(1, "vulnscan: Custom database link defined as " .. db_link)
	end

	if nmap.registry.args.vulnscandb then
		stdnse.print_debug(1, "vulnscan: Using single mode db " .. nmap.registry.args.vulnscandb .. " ...")
		vul = find_vulnerabilities(prod, ver, nmap.registry.args.vulnscandb)
		if #vul > 0 then
			s = s .. nmap.registry.args.vulnscandb
		...
		stdnse.print_debug(1, "vulnscan: Using multi db mode (" .. #db .. " databases) ...")
		for i,v in ipairs(db) do
			...
			stdnse.print_debug(1, "vulnscan: " .. #vul .. " matches in " .. v.file)
		end

		stdnse.print_debug(1, "vulnscan: " .. v_count .. " matches in total")
	end
...
function find_vulnerabilities(prod, ver, db)
	...
	-- Load database
	local v_entries = read_from_file("scripts/vulnscan/" .. db)
	...
	stdnse.print_debug(1, "vulnscan: Starting search of " .. prod ..
		" in " .. db ..
		" (" .. #v_entries .. " entries) ...")
		...
		if type(v_title) == "string" then
			...
			for j=1, #prod_words, 1 do
				...
				if type(v_found) == "number" then
					...
					stdnse.print_debug(2, "vulnscan: Match v_id " .. v_id ..
						" -> v[" .. #v .. "] " ..
						"(" .. v[#v].matches .. " match) " ..
						"(Prod: " .. prod_words[j] .. ")")
				end
			...
			if nmap.registry.args.vulnscanversiondetection ~= "0" and ver ~= nil and ver ~= "" then
...
function prepare_result(v, struct, link)
	...
	if #v > 0 then
		...
		stdnse.print_debug(2, "vulnscan: Maximum matches of a finding are " ..
			...
			for matchpoints=match_max, 1, -1 do
				for i=1, #v, 1 do
					if v[i].matches == matchpoints then
						stdnse.print_debug(2, "vulnscan: Setting up result id " .. i)
						s = s .. report_parsing(v[i], struct, link)
					end
				end

				if nmap.registry.args.vulnscanshowall ~= "1" and s ~= "" then
					-- If the next iteration shall be approached (increases matches)
					if grace == 0 then
						stdnse.print_debug(2, "vulnscan: Best matches found in 1st pass. Going to use 2nd pass ...")
						grace = grace+1
					elseif nmap.registry.args.vulnscanshowall ~= "1" then
						break
					end
				end
			end
...
function read_from_file(file)
	...
	if filepath then
		...
		if not f then
			stdnse.print_debug(1, "vulnscan: Failed to open file" .. file)
		end
		...
		stdnse.print_debug(1, "vulnscan: File " .. file .. " not found")
		return ""
	end
end
